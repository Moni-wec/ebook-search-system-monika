...
# CONFIG (Render Safe)
# =================================
# Use workspace-relative directories instead of /tmp to support Windows
...
def view_book(filename):
    ...
    if filename not in user_files:
        abort(404)
    # safe send from directory with correct folder
...
def upload_page():
    ...
    if request.method == "POST":
        ...
        if not allowed_file(file.filename):
            return render_template("upload.html", message="Only PDF files are allowed")

        try:
            # prevent directory traversal and weird names
            ...
            # derive a normalized book name from the sanitized filename
            ...
            # read existing index; recover from invalid json by resetting
            try:
                with open(DATA_FILE, "r", encoding="utf8") as f:
                    all_books = json.load(f)
                if not isinstance(all_books, list):
                    raise ValueError("data file not list")
            except Exception:
                all_books = []
            ...
            return render_template("upload.html", message="The book was indexed successfully")
...
def clear_session():
    ...
    if user_files:
        ...
        for book in all_books:
                if book.get("book") + ".pdf" in user_files:
                    file_path = os.path.join(UPLOAD_FOLDER, book.get("book") + ".pdf")
                    if os.path.exists(file_path):
                        os.remove(file_path)
                else:
